 ...
 * @note first cannot be const as we need to update GoodsEntry::last_component
...
void LinkGraph::NextComponent()
{
	...
	/* In each run over the link graph at most half of the available component
	 * IDs can be assigned. That's 32768. There is one special
	 * INVALID_COMPONENT_ID which should not be used, so let's say 32767. A new
	 * component ID is only issued if a station with a link for the respective
	 * cargo is found. That means there have to be at least two stations in
	 * that component. Each station can only be in one component at a time.
	 * There can be up to 65534 stations in the game:
	 * 2 ^ 16 available IDs - 2 special ones; INVALID_STATION and NEW_STATION.
	 * So we need at most 65534 / 2 == 32767 component IDs in each run. Thus,
	 * the component IDs will not overflow. There's an assert for that in
	 * LinkGraphComponent::Init(). */
	LinkGraphComponentID current_component_id = this->LinkGraphComponent::index + 2;
	...
	do {
		if (++this->current_station_id >= Station::GetPoolSize()) {
			...
			current_component_id = (current_component_id + 1) % 2;
		}

		/* Find first station of next component */
		Station *station = Station::GetIfValid(this->current_station_id);
		if (station == NULL) continue;
		const GoodsEntry &ge = station->goods[this->cargo];

		/* Same divisibility by 2: We've seen that station before. */
...
void OnTick_LinkGraph()
{
	...
	/* This creates a round robin distribution of all link graphs' turns over the
	 * available dates. */
...
void LinkGraphComponent::Init(LinkGraphComponentID id)
{
	...
	/* -1 as we won't need the last even numbered ID anyway and IDs are
	 * incremented by 2 with alternating divisibility by 2. Thus, if we just
	 * asserted on INVALID_LINKGRAPH_COMPONENT we might not catch an overflow
	 * in the even numbered case. */
	assert(id < INVALID_LINKGRAPH_COMPONENT - 1);
	this->index = id;
 ...
 * @param graph Link graph component this node belongs to.
...
void InitializeLinkGraphs()
{
	...
	/* It's easier to determine "has already a path" from the flows than from
	 * the paths. In MCF2ndPass we only add new paths to edges which already
	 * have a path. As there is no additional cost in clearing away the paths
	 * from MCF1stPass early we just do that here to get the flows. */
