...
private:
	LinkGraph *graph; ///< Link graph we're working with.
	NodeID from;      ///< Current "from" node.
	NodeID to;        ///< Current edge's "to" node.
...
public:
	 ...
	 */
	GraphEdgeIterator(LinkGraphJob *job) : graph(&job->Graph()),
			from(INVALID_NODE), to(INVALID_NODE)
	{}
	...
	NodeID Next()
	{
		if (this->to != INVALID_NODE) {
			this->to = this->graph->GetEdge(this->from, this->to).next_edge;
		}
		return this->to;
	}
};
...
public:
	...
	{
		for (NodeID i = 0; i < job->Graph().GetSize(); ++i) {
			this->station_to_node[job->Graph().GetNode(i).station] = i;
		}
	}
	...
	void SetNode(NodeID source, NodeID node)
	{
		...
		FlowStatMap &flows = this->job->GetNode(node).flows;
		FlowStatMap::const_iterator it = flows.find(this->job->Graph().GetNode(source).station);
		if (it != flows.end()) {
...
void MultiCommodityFlow::Dijkstra(NodeID source_node, PathVector &paths)
{
	...
	while (!annos.empty()) {
		...
		for (NodeID to = iter.Next(); to != INVALID_NODE; to = iter.Next()) {
			...
			EdgeAnnotation &edge_anno = this->job->GetEdge(from, to);
			if (dest->IsBetter(source, capacity, capacity - edge_anno.flow, distance)) {
				annos.erase(dest);
				dest->Fork(source, capacity, capacity - edge_anno.flow, distance);
				annos.insert(dest);
			}
		}
	}
 ...
 */
uint MultiCommodityFlow::PushFlow(EdgeAnnotation &edge, Path *path, uint accuracy,
		uint max_saturation)
{
...
void MCF1stPass::EliminateCycle(PathVector &path, Path *cycle_begin, uint flow)
{
	...
	do {
		...
		cycle_begin = path[cycle_begin->GetNode()];
		EdgeAnnotation &edge = this->job->GetEdge(prev, cycle_begin->GetNode());
		edge.flow -= flow;
	} while (cycle_begin != cycle_end);
}
...
bool MCF1stPass::EliminateCycles(PathVector &path, NodeID origin_id, NodeID next_id)
{
	...
	if (at_next_pos == NULL) {
		 ...
		 * in one path each. */
		PathSet &paths = this->job->GetNode(next_id).paths;
	...
	do {
		...
		for (NodeID source = 0; source < size; ++source) {
			...
			for (NodeID dest = 0; dest < size; ++dest) {
				EdgeAnnotation &edge = this->job->GetEdge(source, dest);
				if (edge.unsatisfied_demand > 0) {
					...
					if (path->GetFreeCapacity() > 0 && this->PushFlow(edge, path,
						 ...
						 * find more. */
						more_loops = more_loops || (edge.unsatisfied_demand > 0);
