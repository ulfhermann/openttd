...
public:
	 ...
	 * @param to_anno Annotation to get undelivered_suppy from.
	 */
	inline bool HasDemandLeft(Node &to, NodeAnnotation &to_anno)
	{
		return (to.supply == 0 || to_anno.undelivered_supply > 0) && to.demand > 0;
	}
...
public:
	 ...
	 */
	virtual void Run(LinkGraphJob *job) { DemandCalculator c(job); }
