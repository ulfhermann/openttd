...
protected:
	MultiCommodityFlow(LinkGraphJob *job) : job(job), graph(&job->Graph()),
	...
	LinkGraph *graph;    ///< Component we're working with.
...
public:
	 ...
	 */
	virtual void Run(LinkGraphJob *job) { Tpass pass(job); }
