...
{
	if (job->Graph().GetNode(from_id).demand > 0) {
		uint demand_back = demand_forw * this->mod_size / 100;
		uint undelivered = job->GetNode(to_id).undelivered_supply;
...
void DemandCalculator::CalcDemand(LinkGraphJob *job, Tscaler scaler)
{
	...
	while (!supplies.empty() && !demands.empty()) {
		...
		NodeAnnotation &from_anno = job->GetNode(from_id);
		...
		for (uint i = 0; i < num_demands; ++i) {
			...
			int32 distance = this->max_distance - (this->max_distance -
					(int32)graph->GetEdge(from_id, to_id).distance) * this->mod_dist / 100;
			...
			demand_forw = min(demand_forw, from_anno.undelivered_supply);

			scaler.SetDemands(job, from_id, to_id, demand_forw);

			if (scaler.HasDemandLeft(to, job->GetNode(to_id))) {
				demands.push_back(to_id);
			} else {
				num_demands--;
			}

			if (from_anno.undelivered_supply == 0) break;
		}

		if (from_anno.undelivered_supply != 0) {
	...
	max_distance(MapSizeX() + MapSizeY() - 2)
{
	...
	CargoID cargo = job->Graph().GetCargo();
