...
void LinkGraphOverlay::RebuildCache()
{
	...
	FOR_ALL_STATIONS(sta) {
		...
		FOR_EACH_SET_CARGO_ID(c, this->cargo_mask) {
			...
			LinkGraph *lg = LinkGraph::GetIfValid(sta->goods[c].link_graph);
			if (lg == NULL) continue;

			NodeID from_node = sta->goods[c].node;
			supply += lg->GetNode(from_node).MonthlySupply(lg->GetLastCompression());
			for (NodeID i = lg->GetFirstEdge(from_node); i != INVALID_NODE; i = lg->GetNextEdge(from_node, i)) {
				if (lg->GetEdge(from_node, i).capacity == 0) continue;

				StationID to = lg->GetNode(i).station;
...
void LinkGraphOverlay::AddLinks(const Station *from, const Station *to)
{
	...
	FOR_EACH_SET_CARGO_ID(c, this->cargo_mask) {
		...
		const LinkGraph *lg = LinkGraph::GetIfValid(from->goods[c].link_graph);
		if (lg == NULL) continue;
		NodeID source = from->goods[c].node;
		for (NodeID i = lg->GetFirstEdge(source); i != INVALID_NODE; i = lg->GetNextEdge(source, i)) {
			const Edge &edge = lg->GetEdge(source, i);
			this->AddStats(edge.MonthlyCapacity(lg->GetLastCompression()),
					edge.MonthlyUsage(lg->GetLastCompression()), sum_flows,
