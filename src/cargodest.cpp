...
void UpdateExpectedLinks(Industry *ind)
{
	for (uint i = 0; i < lengthof(ind->produced_cargo); i++) {
		...
		if (CargoHasDestinations(cid)) {
			...
			num_links += ind->last_month_production[i] / _settings_game.economy.cargodest.cargo_scale_ind[IsTownCargo(cid) ? CARGO_SCALE_IND_TOWN : CARGO_SCALE_IND];
...
/* virtual */ uint Industry::GetDestinationWeight(CargoID cid, byte weight_mod) const
{
	...
	weight += (this->last_month_production[0] + this->last_month_production[1]) * weight_mod / _settings_game.economy.cargodest.weight_scale_ind[WEIGHT_SCALE_IND_PROD];
