...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	for (Vehicle *v = front; v != NULL; v = v->Next()) {
		...
		if (front->current_order.IsRefit() && artic_part == 1 && IsArticulatedVehicleEmpty(v) &&
			...
			if (new_cid < NUM_CARGO && (new_cid != v_start->cargo_type || new_subtype != v_start->cargo_subtype)) {
		...
		assert(v->cargo_cap >= v->cargo.OnboardCount());
		/* If there's goods waiting at the station, and the vehicle
		 * has capacity for it, load it on the vehicle. */
		uint cap_left = v->cargo_cap - v->cargo.OnboardCount();
