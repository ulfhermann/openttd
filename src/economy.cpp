	...
	for (; v != NULL; v = v->Next()) {
		...
		if (HasBit(v->vehicle_flags, VF_CARGO_UNLOADING) && (u->current_order.GetUnloadType() & OUFB_NO_UNLOAD) == 0) {
			...
			if (CargoHasDestinations(v->cargo_type)) {
				...
				remaining = v->cargo.MoveTo(&ge->cargo, amount_unloaded, mta, payment, last_visited, last_order, &did_transfer);
		...
		if (!ge->cargo.Empty() && cap_left > 0) {
			...
			ge->cargo.MoveTo(&v->cargo, cap, StationCargoList::MTA_CARGO_LOAD, NULL, st->xy, last_order);
